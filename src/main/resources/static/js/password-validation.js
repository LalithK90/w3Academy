(function($) {
	$.fn.extend({
		passwordValidation: function(_options, _callback, _confirmcallback) {
			...
			let _cases = buildPasswordRegex();

			let _element = this;
			let $confirmField = (_options.confirmField !== undefined)? $(_options.confirmField): undefined;
			...
			function validateField() {
				...
				//Determine if valid
				let validPassword = (failedCases.length === 0) && ($(_element).val().length >= _options.minLength);
